...
public class NetworkDecoder extends LengthFieldBasedFrameDecoder {
    ...
    private Object decodeBody(ByteBuf in) {
        ...
        // check version
        if (header[4] != 1) {
            throw new IllegalStateException("Versions are not consistent, expect version: [" + 1 + "], actual version: [" + header[4] + "]");
        }
        ...
        return JSONObject.parseObject(in.toString(Charset.defaultCharset()), FastMessage.class);
    }
}
